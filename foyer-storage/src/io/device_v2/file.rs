...
use crate::{
    IoEngineConfig, PsyncIoEngineConfig, RawFile,
    io::{
        PAGE,
...
pub struct FileDeviceBuilder {
    io_engine: Option<Arc<dyn IoEngineV2>>,
    path: PathBuf,
...
impl FileDeviceBuilder {
    ...
    pub fn with_io_engine(mut self, io_engine: Arc<dyn IoEngineV2>) -> Self {
...
impl DeviceBuilder for FileDeviceBuilder {
    fn build(self) -> Result<Arc<dyn Device>> {
        ...
        let io_engine = match self.io_engine {
            ...
            None => {
                let io_engine = PsyncIoEngineConfig::new().boxed().build(ctx).await?;
                Arc::new(io_engine)
            }
        };
