...
use crate::{
    ...
    io::{
        ...
        engine_v2::{IoEngine, IoEngineConfig, IoHandle},
    },
};
...
impl IoEngineConfig for UringIoEngineConfig {
    fn build(self: Box<Self>) -> BoxFuture<'static, Result<Arc<dyn IoEngine>>> {
        async move {
            if self.threads == 0 {
...
impl UringIoEngine {
    ...
    fn read(&self, buf: Box<dyn IoBufMut>, raw: RawFile, offset: u64, thread: usize) -> IoHandle<Box<dyn IoBufMut>> {
        let (tx, rx) = oneshot::channel();
        let shard = &self.read_txs[thread];
        let (ptr, len) = buf.as_raw_parts();
        let rbuf = RawBuf { ptr, len };
        let addr = RawFileAddress { raw, offset };
        #[cfg(feature = "tracing")]
    ...
    fn write(&self, buf: Box<dyn IoBuf>, raw: RawFile, offset: u64, thread: usize) -> IoHandle<Box<dyn IoBuf>> {
        let (tx, rx) = oneshot::channel();
        let shard = &self.write_txs[thread];
        let (ptr, len) = buf.as_raw_parts();
        let rbuf = RawBuf { ptr, len };
        let addr = RawFileAddress { raw, offset };
        #[cfg(feature = "tracing")]
...
impl IoEngine for UringIoEngine {
    fn read(&self, buf: Box<dyn IoBufMut>, raw: RawFile, offset: u64) -> IoHandle<Box<dyn IoBufMut>> {
        ...
        let thread = ops % self.read_txs.len();
        self.read(buf, raw, offset, thread)
    }

    fn write(&self, buf: Box<dyn IoBuf>, raw: RawFile, offset: u64) -> IoHandle<Box<dyn IoBuf>> {
        let ops = self.ops.fetch_add(1, Ordering::Relaxed);
        let thread = ops % self.write_txs.len();
        self.write(buf, raw, offset, thread)
    }
}
