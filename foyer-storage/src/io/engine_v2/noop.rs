...
impl IoEngine for NoopIoEngine {
    fn read(&self, buf: Box<dyn IoBufMut>, _: RawFile, _: u64) -> IoHandle<Box<dyn IoBufMut>> {
        async move { (buf, Ok(())) }.boxed().into()
    }

    fn write(&self, buf: Box<dyn IoBuf>, _: RawFile, _: u64) -> IoHandle<Box<dyn IoBuf>> {
        async move { (buf, Ok(())) }.boxed().into()
    }
}
