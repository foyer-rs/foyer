...
type IoHandleInner<B> = BoxFuture<'static, (B, Result<()>)>;
#[cfg(feature = "tracing")]
type IoHandleInner<B> = InSpan<BoxFuture<'static, (B, Result<()>)>>;
/// A detached I/O handle that can be polled for completion.
#[pin_project]
pub struct IoHandle<B> {
    #[pin]
    inner: IoHandleInner<B>,
    callback: Option<Box<dyn FnOnce() + Send + 'static>>,
}

impl<B> Debug for IoHandle<B> {
...
#[cfg(not(feature = "tracing"))]
impl<B> From<BoxFuture<'static, (Box<dyn IoB>, Result<()>)>> for IoHandle<B> {
...
#[cfg(feature = "tracing")]
impl<B> From<BoxFuture<'static, (B, Result<()>)>> for IoHandle<B> {
...
pub trait IoEngine: Send + Sync + 'static + Debug {
    /// Open a file from the given path and options.
    fn open(&self, path: Box<dyn AsRef<Path>>, options: OpenOptions) -> BoxFuture<'static, Result<File>>;
    /// Read data into the buffer from the specified partition and offset.
    fn read(&self, buf: Box<dyn IoBufMut>, file: &File, offset: u64) -> IoHandle<Box<dyn IoBufMut>>;
    /// Write data from the buffer to the specified block and offset.
    fn write(&self, buf: Box<dyn IoBuf>, file: &File, offset: u64) -> IoHandle<Box<dyn IoBuf>>;
}
