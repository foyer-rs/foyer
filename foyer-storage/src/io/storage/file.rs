...
use crate::{
    IoEngineConfig, RawFile, Statistics, Throttle,
    engine::block::storage::{BlockEngineStorage, BlockEngineStorageBuildCtx, BlockEngineStorageConfig, BlockStorage},
    io::{
        PAGE,
        engine_v2::{IoEngine, IoEngineConfig as _, monitor::MonitoredIoEngine, psync::PsyncIoEngineConfig},
    },
};
...
impl FileStorageBuilder {
    ...
    /// Set the size of the file device.
    ///
    /// The given size may be modified on build for alignment.
    ///
...
impl FileStorageBuilder {
    async fn build_inner(self, metrics: Arc<Metrics>) -> Result<Arc<FileStorage>> {
        ...
        let io_engine = match self.io_engine {
            Some(io_engine) => io_engine,
            None => PsyncIoEngineConfig::new().boxed().build().await?,
        };
        let io_engine = MonitoredIoEngine::new(io_engine, metrics, statistics.clone());
...
pub struct FileStorage {
    io_engine: Arc<MonitoredIoEngine>,
    file: Arc<File>,
...
//         let partition = Arc::new(FileBlock {
...
impl BlockEngineStorageConfig for FileStorageBuilder {
    fn build(
        self: Box<Self>,
        ctx: BlockEngineStorageBuildCtx,
    ) -> BoxFuture<'static, Result<Arc<dyn BlockEngineStorage>>> {
        async move {
            let storage = self.build_inner(ctx.metrics).await?;
            let storage: Arc<dyn BlockEngineStorage> = storage;
            Ok(storage)
        }
        .boxed()
    }
...
impl BlockEngineStorage for FileStorage {
    fn create_block(&self, size: usize) -> Result<Arc<dyn BlockStorage>> {
        if size > self.size {
            return Err(Error::no_space(self.size, 0, size));
        }
