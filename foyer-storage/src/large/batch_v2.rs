...
use crate::{
    error::{Error, Result},
    io::{IoBuffer, OwnedIoSlice, ALIGN},
...
impl Blob {
    ...
    const ENTRY_CAPACITY: usize = (ALIGN - (Self::META_CHECKSUM_BYTES + Self::SIZE_BYTES + Self::ENTRY_COUNT_BYTES))
...
impl EntryWriter for BlobWriter {
    ...
    fn finish(mut self) -> (OwnedIoSlice, Option<Blob>) {
        ...
        let checksum = Checksummer::checksum32(&self.io_slice[Blob::META_CHECKSUM_BYTES..ALIGN]);
...
mod tests {
    ...
    fn test_blob_serde() {
        ...
        let io_slice = buf.into_owned_slice().slice(..14 * KB);
        assert_eq!(io_slice.len(), 14 * KB);

        // 4K (meta)
        let mut writer = BlobWriter::new(io_slice, Arc::new(Metrics::noop()));
        ...
        // 8K (deny)
        let op = writer.push(&2, &vec![2u8; 7 * KB], 2, Compression::None, 2);
        ...
        let (io_slice, blob) = writer.finish();
        assert_eq!(io_slice.len(), 2 * KB);
    ...
    fn test_window_serde() {
        ...
        let io_slice = buf.into_owned_slice().slice(..64 * KB);
        ...
        let io_slice = buf.into_owned_slice().slice(60 * KB..);
        assert_eq!(io_slice.len(), 4 * KB);
