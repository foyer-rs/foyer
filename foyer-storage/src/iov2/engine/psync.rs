...
impl PsyncIoEngine {
    ...
    fn read(&self, buf: Box<dyn IoB>, file: FileHandle, offset: u64) -> IoHandle {
    ...
    fn write(&self, buf: Box<dyn IoB>, file: FileHandle, offset: u64) -> IoHandle {
        let runtime = self.spawner.clone();
...
impl IoEngine for PsyncIoEngine {
    #[cfg_attr(
        feature = "tracing",
        fastrace::trace(name = "foyer::storage::io::engine::psync::submit")
    )]
    fn submit(&self, unit: IoUnit) -> IoHandle {
        let IoTarget::File { raw, offset } = unit.target else {
            ...
            panic!("psync io engine only supports file io");
        };

        let file = FileHandle::from(raw);

        match unit.op {
            IoOp::Read => self.read(unit.buf, file, offset),
            IoOp::Write => self.write(unit.buf, file, offset),
        }
    }
}
