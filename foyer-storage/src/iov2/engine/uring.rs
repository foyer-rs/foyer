...
use crate::iov2::{
    bytes::IoB,
    engine::{IoEngine, IoEngineBuildContext, IoEngineConfig, IoHandle, IoOp, IoTarget, IoUnit, RawFile},
};
...
impl UringIoEngine {
    ...
    fn read(&self, buf: Box<dyn IoB>, file: RawFile, offset: u64, tid: usize) -> IoHandle {
        let (tx, rx) = oneshot::channel();
        let shard = &self.read_txs[tid];
    ...
    fn write(&self, buf: Box<dyn IoB>, file: RawFile, offset: u64, tid: usize) -> IoHandle {
        let (tx, rx) = oneshot::channel();
        let shard = &self.write_txs[tid];
        let (ptr, len) = buf.as_raw_parts();
        let rbuf = RawBuf { ptr, len };
        let addr = RawFileAddress { file, offset };
        #[cfg(feature = "tracing")]
...
impl IoEngine for UringIoEngine {
    fn submit(&self, unit: IoUnit) -> IoHandle {
        let IoTarget::File { raw, offset } = unit.target else {
            // FIXME(MrCroxx): Refine the hint. Raise error or panic?
            panic!("uring io engine only supports file io");
        };
        let seq = self.sequence.fetch_add(1, Ordering::Relaxed);

        match unit.op {
            IoOp::Read => self.read(unit.buf, raw, offset, seq % self.read_txs.len()),
            IoOp::Write => self.write(unit.buf, raw, offset, seq % self.write_txs.len()),
        }
    }
}
